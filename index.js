...
// Basic route
app.get('/', (req, res) => {
    res.send('🌧️ Hangout server running');
});
...
io.on('connection', (socket) => {
    ...
    socket.on('join_lobby', ({ username }) => {
        ...
        if (!trimmed) {
            return socket.emit('error', { msg: 'Username required' });
        }
    ...
    socket.on('create_room', () => {
        ...
        if (!username) {
            return socket.emit('error', { msg: 'Not in lobby' });
        }

        if (socketToRoom.has(socket.id)) {
            return socket.emit('error', {
                msg: 'Already in a room',
            });
        }
        ...
        socket.emit('room_joined', {
            ...
            player,
            players: [player],
            musicState: rooms.get(code).musicState,
        });
    });
    ...
    socket.on('join_room', ({ code }) => {
        ...
        if (socketToRoom.has(socket.id)) {
            return socket.emit('error', {
                msg: 'You are already in a room. Leave it first.',
            });
        }
        ...
        if (existingPlayer) {
            return socket.emit('error', {
                msg: 'That username is already taken in this room',
            });
        }
    ...
    socket.on('equip', ({ hat, pet }) => {
        ...
        io.to(roomCode).emit('player_equipped', {
            id: socket.id,
            hat: player.hat,
            pet: player.pet,
        });
...
server.listen(PORT, () => {
    console.log(`🌧️ Hangout server running on port ${PORT}`);
});